...
func markdown(c catalog) string {
	...
	// GitHub Pages renders docs/ with Jekyll and just-the-docs builds its sidebar
	// from front matter. These files are rewritten on every run, so the front
	// matter is emitted here rather than pasted into the page.
	b.WriteString("---\ntitle: Data schema\nparent: API and data\nnav_order: 10\n---\n\n")
