...
| 5 | ✅ done 2026-08-25 | web microfrontend shell: host `apps/web` + remotes top-level `web-auth`/`web-admin-users`/`web-admin-roles` (federation runtime-configurable via `/config.js`, shared react/react-dom/query singleton), `@starter/ui` primitives + Tailwind v4 tokens, `packages/contracts` = openapi-typescript dari aggregate spec statis (`scripts/compose-specs.mjs`, 19 paths) + wrapper openapi-fetch (bearer attach, silent-refresh 401→retry-once), RequirePermission guard (UI hint) + bootstrap sesi via refresh cookie saat mount, TanStack Query + docs QUERY_KEYS.md, Vitest+RTL 1 test per remote (vi.mock api) + MSW mock mode offline dev, Biome clean, bundle-budget gate host (395KB<400KB), **Playwright smoke login→admin table→logout PASS vs mesh nyata** (PG+Redis docker + 4 Go services + gateway + 4 preview apps; `scripts/e2e-mesh.sh ci`), CI job web+playwright, ops set per app (Dockerfile nginx+config-entrypoint/Jenkinsfile via goPlatformWeb/compose/k8s{deployment,service,hpa}), TOKEN_POLICY.md |
| 6 | ✅ done 2026-08-25 | observability & ops: OTel SDK semua service (OTLP, noop tanpa `OTEL_EXPORTER_OTLP_ENDPOINT`), gateway parent span + propagasi traceparent ke upstream, trace_id di slog tiap request; middleware Trace masuk stack standar NewRouter; slow-request log env-tunable (`SLOW_REQUEST_THRESHOLD_MS`) + slow-query bridge sudah ada; compose.observability.yml profile obs (otel-collector + prometheus scrape 6 service + grafana provisioned dashboard rps/p95/error-ratio/stream-lag/realtime conns); audit-viewer endpoint di worker (`GET /api/v1/audit/viewer`, paginated, guard `audit:read:any` + internal-secret) + spec + test integrasi + worker masuk default UPSTREAMS; error-reporter port noop + adapter Sentry opsional `SENTRY_DSN`, dipanggil dari Recoverer; SCALING.md (pool, redis, HPA math, perf baseline terukur, trigger shard); ONBOARDING.md (2 toolchain, tabel port); ARCHITECTURE.md (mermaid: topology/schemas/streams/federation/observability); Bruno collection 11 request full journey |
| 7 | ✅ done 2026-08-25 | hardening: depguard terverifikasi CI (golangci) + import-boundary checker web (`scripts/check-deps.mjs`, negative-tested); security headers helmet-parity masuk stack standar + CSRF posture terdokumentasi (docs/SECURITY.md — POST-only+JSON+SameSite=Lax); CI security job Trivy (vuln+misconfig, HIGH/CRITICAL failing) + semgrep p/default; perf smoke tool (`scripts/perf-smoke`, tanpa deps) → baseline nyata tercatat di SCALING.md (healthz 9.2k rps p50 1.3ms; login bcrypt 389 rps p95 225ms); resilience drill (`scripts/resilience-drill.sh`) dijalankan: kill auth under load → 503 envelope standar → restart → recover **PASS** — drill menemukan & memfix bug nyata session endpoints auth (middleware masih verifikasi bearer padahal gateway sudah strip Authorization per kontrak item 32 → kini identity-header based); Deprecation helper RFC-9745 + docs/API_VERSIONING.md (freeze /api/v1); README final rewrite + `infra/compose.base.yml` (mesh lengkap + seeders one-shot); release automation `.github/workflows/release.yml` (release-please) + image tags per komponen via Jenkinsfiles; docs/DOD.md checklist tereksekusi & signed off |
| — | | |
