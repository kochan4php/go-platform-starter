...
| **prod** | Production VPS deployment | production compose | `./scripts/deploy-prod.sh` |
...
│   ├── deploy-uat.sh · deploy-demo.sh · deploy-prod.sh
...
```bash
sudo DOMAIN=example.com AGE_BACKUP_RECIPIENT=age1... ./scripts/deploy-prod.sh
```
...
| `EDGE_PORT` | no | host port for the plain HTTP edge listener (default 80) |
| `APP_PUBLIC_URL` | auto | browser-facing auth URL; switch to `https://` with TLS |
<!-- nosemgrep: javascript.lang.security.detect-insecure-websocket.detect-insecure-websocket -- documentation -->
...
Template for all protected targets: `infra/.env.production.example`; set
`STACK_ENV` to `uat`, `demo`, or `prod` in the copied target file.
